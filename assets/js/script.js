...
(function ($) {
    ...
    /*=====================
      ...
      //send message and link
      $('#messengerForm').on('submit', function(event){
        event.preventDefault();

          var message = $('#setemoj').val();
          var sender = $('.submit').attr('current-user');
          var receiver = $('.submit').attr('receiver-user');

          if($.trim(message) == '') {
              var errmsg = "Please write something to send!!";
              return false;
          }
          var link = $('#linkPlaceholder').val();
      
         $.ajax({
          url:"upload-link.php",
          method:"POST",
          data: 'message='+message+'&link='+link+'&sender='+sender+'&receiver='+receiver,
          dataType:"JSON",
          cache: false,
          success: function (data)
            {
                if(data.type == 'error')
                {
                    ...
                    // load_message(sender, receiver)
                    // $(".chatappend ul li:last").focus();
                    load_sticker(sender, receiver)
                    $('#setemoj').val('');
                    console.log(data.text)
                    $(".messages").animate({ scrollTop: $(document).height() }, "fast");
                }
          ...
          //  if(data.error != '')
          //  {
          //   $('#message-send')[0].reset();
          //   //$('#message-error').html(data.error);
          //   $("#display-message").animate({ 
          //      scrollTop: $( 
          //        '#display-message').get(0).scrollHeight 
          //  }, 2000);
          //   load_message();
          //  }
          // }
      ...
      $('.user_id').on('click', function () {
        ...
        $('.custom-scroll').removeClass("active");
        var active_class = $(this).attr("data-to");
        $('#' + active_class).addClass("active");

        var sender = $('.user_id').attr('current-user');
        var receiver = $('.user_id').attr('selected-user');
        load_sticker(sender, receiver)
        $(".messages").animate({ scrollTop: $(document).height() }, "fast");
    ...
    /*=====================
       ...
       $('.sticker-contain ul li').on('click', function (e) {
        ...
        var sender = $('.sticker-contain').attr('sender');
        var receiver = $('.sticker-contain').attr('receiver');
        ...
        //ajax code for uploading stickers
         $.ajax({
           url:"upload-message.php",
           method:"POST",
           dataType:"JSON",
           data: 'sticker='+sticker+'&sender='+sender+'&receiver='+receiver,
           cache: false,
           success: function (data)
            {
                if(data.type == 'error')
    ...
    function load_sticker(data1, data2){
        $.ajax({
        url:"load-messages.php",
        method:"POST",
        dataType:"JSON",
        async: false,
        data: 'sticker_sender='+data1+'&sticker_receiver='+data2, 
        
        success:function(response){
             $('.messages .chatappend').append(response); 
    ...
    $('.add-link').on('click', function (e) {
        ...
        $(this).toggleClass("active");
        var link = $('#linkPlaceholder').val();
        $('#setemoj').css('display', 'none');
        $('#setemoj').val('null');
        $('#linkPlaceholder').css('display', 'block');
        $('#send-msg').removeClass('disabled').removeAttr("disabled")
    });
